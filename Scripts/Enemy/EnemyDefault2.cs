...
public class EnemyDefault2 : MonoBehaviour
{
    ...
    private Coroutine throwGranadeCoroutine;
    ...
    private readonly float granadeCooldown = 1.2f;
    private readonly int granadeVelocity = 15;
    ...
    {
        if (!isShooting && throwGranadeCoroutine != null)
        {
            throwGranadeCoroutine = StartCoroutine(SpawnGranade());
        }
    }

    private void OnDisable()
    {
        if (throwGranadeCoroutine != null)
        {
            StopCoroutine(throwGranadeCoroutine);
            isShooting = false;
        }
    }
    ...
    IEnumerator SpawnGranade()
    {
        isShooting = true;

        isLocked = true;

        yield return new WaitForSeconds(granadeCooldown - difficulty / 5f);

        GameObject pooledProjectile = GranadePoolerEnemy2.SharedInstance.GetPooledObject();
        if (pooledProjectile != null)
        {
            pooledProjectile.SetActive(true);
            pooledProjectile.transform.position = projectileSpawner.transform.position;
            pooledProjectile.GetComponent<Rigidbody>().velocity = projectileSpawner.transform.forward * granadeVelocity;
        }
